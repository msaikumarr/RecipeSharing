...
export const RecipeProvider = ({ children }) => {
  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        const response = await fetch("https://recipesharing-mzt5.onrender.com/api/recipes");
  ...
  const fetchRecipesByState = async (state) => {
    try {
      ...
      const response = await fetch(`https://recipesharing-mzt5.onrender.com/api/recipes/${encodeURIComponent(state)}`);
      const data = await response.json();
  ...
  const fetchUserRecipes = useCallback(async () => {
    try {
      ...
      const response = await fetch("https://recipesharing-mzt5.onrender.com/api/recipes/my-recipes", {
        method: "GET",
        headers: {
